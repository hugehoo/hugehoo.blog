...
// revenueProvider 인터페이스? 그런 건 모른다
func (a *AdsenseClient) GetRevenue() ([]AdRevenue, error) {
...
// AdmobClient도 마찬가지. 인터페이스의 존재를 알 필요가 없다
...
import (
    "adplatform/adsense"
    "adplatform/admob"
    "adplatform/dashboard"
    "fmt"
)

func main() {
    // AdSense 수익 조회
    fmt.Println("=== AdSense Revenue ===")
    adsenseClient := adsense.NewAdsenseClient()
    d1 := dashboard.NewDashboard(adsenseClient)
    d1.PrintRevenue()

    // AdMob 수익 조회 - 동일한 Dashboard, 다른 구현체
    fmt.Println("\n=== AdMob Revenue ===")
    admobClient := admob.NewAdmobClient()
    d2 := dashboard.NewDashboard(admobClient)
    d2.PrintRevenue()
}
```
...
```bash
=== AdSense Revenue ===
2024-01-01: $150.50 USD
2024-01-02: $200.75 USD
2024-01-03: $180.25 USD

=== AdMob Revenue ===
2024-01-01: $320.00 USD
2024-01-02: $450.50 USD
2024-01-03: $380.75 USD
```

Dashboard 코드는 전혀 바뀌지 않았다는 점에 주목하자. `AdsenseClient`를 사용하든 `AdmobClient`를 사용하든 Dashboard 입장에서는 동일한 방식으로 동작한다. 주입하는 구현체만 바꾸면 되기 때문에, 런타임에 동적으로 구현체를 교체하는 것도 가능하다.

---

## 테스트 코드

소비자 측 인터페이스 패턴의 진가는 테스트에서 드러난다. 실제 광고 플랫폼 API에 의존하지 않고도 비즈니스 로직을 검증할 수 있기 때문이다.
...
func (m *mockProvider) GetRevenue() ([]adsense.AdRevenue, error) {
    return m.revenues, m.err
}

func TestPrintRevenue_Success(t *testing.T) {
    mock := &mockProvider{
        revenues: []adsense.AdRevenue{
            {Date: "2024-01-01", Amount: 100.0, Currency: "USD"},
        },
    }

    d := dashboard.NewDashboard(mock)
    err := d.PrintRevenue()
...
func TestPrintRevenue_Error(t *testing.T) {
    mock := &mockProvider{
...
위 테스트에서 `mockProvider`는 실제 광고 플랫폼 API와 전혀 연결되어 있지 않다. 성공 케이스에서는 미리 정해둔 테스트 데이터를 반환하고, 실패 케이스에서는 에러를 반환하도록 설정했다. 이렇게 하면 네트워크 상태나 API 연결 여부에 관계없이 Dashboard의 비즈니스 로직을 독립적으로 검증할 수 있다.
