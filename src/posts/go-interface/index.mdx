...
하지만 Go에서는 **소비자 측에 인터페이스를 정의하는 것**이 관례이자 권장 패턴이다.
...
이 방식에서는 구현체가 인터페이스의 존재를 **반드시 알고 있어야** 한다. `implements` 키워드를 통해 "나는 이 인터페이스를 구현하겠다"고 명시적으로 선언해야 하기 때문이다. 이는 인터페이스와 구현체 사이에 **강한 계약 관계**가 형성된다는 의미이기도 한데, 구현체를 작성하는 시점에 이미 인터페이스가 존재해야 하고, 구현체는 그 인터페이스에 종속된다.
...
Go는 완전히 다르다. **메서드 시그니처만 일치하면 자동으로 인터페이스를 구현한 것으로 간주**된다.
...
여기서 핵심은 구현체가 인터페이스의 존재를 **전혀 몰라도 된다**는 점이다. `ConvenienceStore`를 작성하는 개발자는 `CustomerGetter`라는 인터페이스가 어딘가에 있는지 없는지 신경 쓸 필요가 없다. 그저 자신에게 필요한 메서드를 정의할 뿐이다.
...
덕분에 새로운 인터페이스를 도입하더라도 기존 코드는 전혀 수정할 필요가 없다. 이미 적절한 메서드를 가지고 있다면 자동으로 그 인터페이스를 만족하기 때문이다. 이것이 바로 **인터페이스를 소비자 측에 두는 것이 자연스러운 이유**다.
...
이 다이어그램을 보면 의존성 화살표가 구현체에서 소비자 방향으로 향하고 있다. client 패키지는 store나 mart 패키지를 직접 import할 필요가 없다(Customer 타입 정도만 제외하면). 반대로 store와 mart도 client의 존재를 전혀 모른다.

이런 구조에서는 **순환 의존이 구조적으로 발생할 수 없다.** A가 B를 모르고, B도 A를 모르는데 어떻게 순환이 생기겠는가? 패키지 간 결합도를 낮추는 데 있어서 이보다 깔끔한 방법은 없다.
...
제공자가 인터페이스를 정의하면 보통 "앞으로 필요할지도 모르는" 모든 메서드를 다 넣게 된다. 결과적으로 5개, 10개의 메서드를 가진 거대한 인터페이스가 만들어지고, 정작 소비자는 그중 하나만 사용하는 상황이 벌어진다.

반면 소비자가 인터페이스를 정의하면, **실제로 필요한 메서드만** 포함할 수 있다. Client가 `GetAllCustomers`만 필요하다면 그것만 인터페이스에 넣으면 된다. 이것이 SOLID 원칙 중 하나인 인터페이스 분리 원칙(Interface Segregation Principle)이 자연스럽게 지켜지는 이유다.
