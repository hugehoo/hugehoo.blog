...
- **Header Compression** : 헤더 압축은 헤더 정보를 압축하여 전송 시간을 줄이는 기능이다.

이외에도 http2 의 여러 특징이 있지만, gRPC 의 성능을 높이는 주요한 요인한 위 2가지로 정리할 수 있다.
...
#### **Q. gRPC 와 REST API 의 차이는 뭐가 있을까?**

필자가 알기로 REST API 는 하나의 Design Principle 이지 프레임워크가 아니다.
...
어떤 경우에 gRPC 를 사용하면 좋을까?
우선 **성능이 중요한 경우**, 예를 들어 트래픽이 많고 응답속도가 중요한 경우엔 gRPC 를 선택하는 것이 좋다.
또한 **서로 다른 언어로 개발된 서비스 간** 공통 인터페이스 (protobut IDL) 을 통해 통신하는 경우에도 gRPC 를 선택하는 것이 좋다.
서로 다른 언어로 개발된 서버 간 통신은 REST API(JSON) 로도 가능하지 않냐고?
물론 가능하지만, gRPC 를 사용하는게 더 유리한 이유가 있다.
...
두번째는 자동 코드 생성(stub) 지원 기능인데, 이 기능은 서로 다른 언어로 개발된 서버 간 통신에서 특히 유용하다.
