---
title: 'Feature Flag API의 p99 레이턴시 개선(+ OSS contribute)'
...
개선 과정을 돌이켜보면 두 문제 모두 **mutex를 걸고 I/O를 수행** 했다는 동일 원인으로 귀결되었습니다. 문제 1에서는 mutex를 잡고 최대 7초간 sleep하는 Jitter가, 문제 2에서는 mutex를 잡고 S3 업로드를 수행하는 동기 flush가 원인이었습니다. 규모만 달랐을 뿐 패턴은 동일했고 돌이켜보면 해결책도 단순했습니다.

| 단계 | 조치 | 개선 결과 |
|------|------|-----------|
| 문제 1 | Jitter 비활성화 + export 주기 조절 | max 55초 → ~200ms |
| 문제 2 | AsyncExporter 래퍼 도입 | max 25초 → ~200ms 이내 |

![final.png](/final.png)
평균이나 p95에서는 확인되지 않는 문제가 p99같은 tail latency에서 드러나는 경우 원인은 **타이머 기반 flush, 버퍼 임계치 도달, 조건부 retry** 같은 조건부 경로입니다. tail latency의 원활한 원인 추적을 위해서는 가끔씩 발생하는 특수 경우를 고려하는 것이, 문제를 정확히 파악하는데 도움이 된다는 점을 배울 수 있었습니다.
